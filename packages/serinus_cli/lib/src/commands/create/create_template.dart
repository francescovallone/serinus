

import 'package:mason/mason.dart';

final MasonBundle createApplicationTemplate = MasonBundle.fromJson({
  'files': [
    {
      'path': 'lib/main.dart',
      'data': '',
      'type': 'text'
    },
    {
      'path': 'lib/app_service.dart',
      'data': 'aW1wb3J0ICdwYWNrYWdlOnNlcmludXMvc2VyaW51cy5kYXJ0JzsKCmNsYXNzIEFwcFNlcnZpY2UgZXh0ZW5kcyBTZXJpbnVzU2VydmljZXsKCiAgU3RyaW5nIHBpbmcoKXsKICAgIHJldHVybiAiUG9uZyEiOwogIH0KCn0=',
      'type': 'text'
    },
    {
      'path': 'lib/app_controller.dart',
      'data': '',
      'type': 'text'
    },
    {
      'path': 'lib/app_module.dart',
      'data': '',
      'type': 'text'
    },
    {
      'path': 'pubspec.yaml',
      'data': '',
      'type': 'text'
    },
    {
      'path': 'analysis_options.yaml',
      'data': 'aW5jbHVkZTogcGFja2FnZTpsaW50cy9yZWNvbW1lbmRlZC55YW1s',
      'type': 'text'
    },
    {
      'path': 'README.md',
      'data': 'IyB7e25hbWV9fQoKQSBTZXJpbnVzIGFwcGxpY2F0aW9u',
      'type': 'text'
    },
    {
      'path': '.gitignore',
      'data': '',
      'type': 'text'
    },
  ],
  'name': 'create_serinus_app',
  'version': '0.0.1-dev.1',
  'environment': {'mason': '>=0.1.0-dev <0.1.0'},
  'changelog': {
    'path': 'CHANGELOG.md',
    'data':
        'IyMgMC4wLjEtZGV2LjEKCi0gSW5pdGlhbCB2ZXJzaW9uLgo=',
    'type': 'text'
  },
  'description': 'A Serinus application.',
  'license': {
    'path': 'LICENSE',
    'data': '',
    'type': 'text'
  },
  'vars': {
    'name': {
      'type': 'string',
      'description': 'Your application name',
      'default': 'example',
      'prompt': 'What is your app name?'
    }
  }
});
