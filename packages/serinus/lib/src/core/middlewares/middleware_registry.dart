...
class MiddlewareRegistry extends Provider with OnApplicationBootstrap {
  ...
  final Map<InjectionToken, Set<MiddlewareConfiguration>> _middlewareConfigurations = {};
  ...
  void _addToGraph() {
    final middlewareConfigurations = <MapEntry<InjectionToken, MiddlewareConfiguration>>{};
    ...
    for(final middlewareConfiguration in middlewareConfigurations) {
      ...
      final routes = <RouteInfo>{
        ...middlewareConfiguration.value.routes
      };
      for (final scope in _config.modulesContainer.scopes) {
        controllers.addAll(scope.controllers.where((controller) => middlewareConfiguration.value.controllers.contains(controller.runtimeType) || middlewareConfiguration.value.controllers.isEmpty));
      }
      for (final controller in controllers) {
        routes.addAll(controller.routes.entries.map((e) => RouteInfo(
          path: '${controller.path.stripEndSlash()}${e.value.route.path}'.stripEndSlash().addLeadingSlash(),
          method: e.value.route.method,
          versions: [
            if(e.value.route.version != null) VersioningOptions.uri(version: e.value.route.version!),
            if(controller.version != null) VersioningOptions.uri(version: controller.version!),
            if(_config.versioningOptions != null) _config.versioningOptions!,
          ]
        )));
        if(controller is SseController) {
          routes.addAll(controller.sseRoutes.entries.map((e) => RouteInfo(
            path: '${controller.path.stripEndSlash()}${e.value.route.path}'.stripEndSlash().addLeadingSlash(),
            method: e.value.route.method,
            versions: [
              if(e.value.route.version != null) VersioningOptions.uri(version: e.value.route.version!),
      ...
      routes.removeWhere((route) => middlewareConfiguration.value.excludedRoutes.any((excludeRoute) => _canResolve(route, excludeRoute)));
          ...
          _inspector.graph.insertEntrypoint(
            Entrypoint(
              ...
              metadata: EntrypointMetadata(
                ...
                versions: route.versions?.map((version) => version.version).toList() ?? [],
              )
            )
          );
        }
  ...
  Future<void> _loadMiddlewareConfiguration(InjectionToken moduleToken, Module module) async {
  ...
  Future<void> _mapMiddlewareToRoutes() async {
    final entriesSortedByDistance = [...(_middlewareConfigurations.entries)]..sort(
      (entryA, entryB){
    ...
    for (final scope in _config.modulesContainer.scopes) {
      for(final route in _middlewareByRoute.keys) {
        scope.setRouteMiddlewares(route, (request) {
          final requestRouteInfo = RouteInfo(path: request.uri.path, method: HttpMethod.parse(request.method));
          return _middlewareByRoute[route]?.where((configuration) {
            return configuration.routes.any((routeInfo) => _canResolve(requestRouteInfo, routeInfo)) && !configuration.excludedRoutes.any((excludeRoute) => _canResolve(requestRouteInfo, excludeRoute));
          }).map((configuration) => configuration.middlewares).expand((m) => m).toList() ?? <Middleware>[];
        });
      }
    }
  }

  Future<void> _registerAllConfigs(Set<MiddlewareConfiguration> configuration) async {
    final controllers = <({Controller controller, ModuleScope scope})>[];
    for(final scope in _config.modulesContainer.scopes) {
      controllers.addAll(scope.module.controllers.map((controller) => (
        controller: controller,
        scope: scope
      )));
    }
    final routesToControllers = <({String id, String path, HttpMethod method, Controller controller, int? version, ModuleScope scope})>[];
    ...
    final includedRoutes = <({Controller controller, String id, HttpMethod method, String path, int? version, ModuleScope scope})>{};
    for (final config in configuration) {
      includedRoutes.addAll(routesToControllers.where((c) => config.controllers.any((spec) => spec == c.controller.runtimeType)));
      for(final route in routesToControllers) {
        ...
        final isIncluded = config.routes.any((routeInfo) => _canResolve(currentRouteInfo, routeInfo));

        // Check if route is excluded
        final isExcluded = config.excludedRoutes.any((excludeRoute) => _canResolve(currentRouteInfo, excludeRoute));
  ...
  bool _canResolve(RouteInfo target, RouteInfo resolver) {
    ...
    if ((target.method != resolver.method) && (resolver.method != HttpMethod.all && target.method != HttpMethod.all)) {
      return false;
    }
  ...
  bool _versionsMatch(List<VersioningOptions>? targetVersions, List<VersioningOptions>? resolverVersions) {
    ...
    final resolverVersionNumbers = resolverVersions.map((v) => v.version).toSet();
  ...
  List<Middleware> getRouteMiddlewares(String path) {
    ...
    final requestRouteInfo = RouteInfo(
      path: path.stripEndSlash().addLeadingSlash(),
    );
    return middlewares.where((configuration) {
      return configuration.routes.any((routeInfo) => _canResolve(requestRouteInfo, routeInfo)) && !configuration.excludedRoutes.any((excludeRoute) => _canResolve(requestRouteInfo, excludeRoute));
    }).map((configuration) => configuration.middlewares).expand((m) => m).toList();
  }
