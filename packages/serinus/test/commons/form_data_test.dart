...
class TestController extends Controller{
  ...
  TestController({super.path = '/'}){
    on(TestRoute(path: '/form', method: HttpMethod.post), (context) async => Response.json(
      context.request.body?.formData?.values ?? {}
    ));
  }
          ...
          () {
            ...
            test(
              ...
              () async {
                ...
                request.files.add(http.MultipartFile.fromString('file', 'file.txt', filename: 'file.txt'));
                final response = await request.send();
                final body = await response.stream.transform(Utf8Decoder()).join();
                final json = jsonDecode(body);

                expect(json['fields'], {'foo': 'bar'});
                expect(json['files'], {'file': {'name': 'file.txt', 'contentType': 'text/plain; charset=utf-8', 'data': 'file.txt'}});
              }
            );
          }
        );
