...
class TestController extends Controller {
  TestController({super.path = '/'}) {
    on(TestRoute(path: '/middleware'), (context) async => Response.text('ok!')..headers['x-middleware'] = context.request.headers['x-middleware']);
  }
}
...
void main() {
  group('$Middleware', () {
      ...
      test(
          ...
          () async {
        final response = await http.get(Uri.parse('http://localhost:3003/middleware'),);
