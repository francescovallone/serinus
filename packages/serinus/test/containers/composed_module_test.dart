...
class ParentMultiStepModule extends Module {
  ParentMultiStepModule(List<Module> imports) : super(imports: imports, providers: [
    Provider.composed<P4>(
      (CompositionContext ctx) async => P4(
        ctx.use<P1>(),
        ctx.use<P2>(),
        ctx.use<P3>(),
      ),
...
void main() {
  group('ComposedModule', () {
    test('composed init receives providers from imports and globals', () async {
      final container = ModulesContainer(ApplicationConfig(serverAdapter: _MockAdapter()));
      ...
      await container.registerModules(SimpleMockModuleWithImports(
        imports: [composed],
      ));
  ...
  group('ComposedModule chain', () {
    test('provider produced by ComposedModule A is available to ComposedModule B', () async {
  ...
  group('ComposedModule behavior', () {
    test('ComposedModule.init returns a Module when invoked', () async {
      ...
      final ctx = CompositionContext({
        TestProviderTwo: TestProviderTwo(),
      });
  ...
  group('ComposedModule cycle detection', () {
    test('finalize throws InitializationError on cyclic composed modules', () async {
      final composedA = Module.composed<Module>(
        (CompositionContext ctx) async {
          return ProducedModuleA();
        },
      ...
      expect(() => container.finalize(parent), throwsA(isA<InitializationError>()));
    });
  });
  group('ComposedModule multi-step chain', () {
    test('A -> B -> C chain resolves providers across composed modules', () async {
      final composedA = Module.composed<Module>(
                (ctx) async => ModuleA(), 
                inject: []
              );
  ...
  group('ComposedProvider wrong runtime type', () {
    test('finalize throws InitializationError when composed provider returns wrong type', () async {
      final composed = Module.composed<Module>((ctx) async => ModuleWithWrongComposed(), inject: []);
